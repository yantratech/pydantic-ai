...
def build_run_context(ctx: GraphRunContext[GraphAgentState, GraphAgentDeps[DepsT, Any]]) -> RunContext[DepsT]:
    ...
    # `_output_buffers` (see the invariant on `GraphAgentDeps.loaded_capability_ids`). Never add any of them as a
    # `replace` kwarg — forking the object would silently break in-step capability loads / tool reveals / message
    # enqueues / event delivery / tool-defs caching / buffered output updates.
