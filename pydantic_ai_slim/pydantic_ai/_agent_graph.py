...
def build_run_context(ctx: GraphRunContext[GraphAgentState, GraphAgentDeps[DepsT, Any]]) -> RunContext[DepsT]:
    ...
    # `_mcp_tool_defs_cache`, `_durable_operations`, `_run_capabilities_by_id`, `_output_buffers`
    # (see the invariant on `GraphAgentDeps.loaded_capability_ids`). Never add any of them as a
    # `replace` kwarg -- forking the object would break in-step capability loads / tool reveals /
    # message enqueues / cancellation / event delivery / tool-defs caching / durable operation
    # dispatch / buffered output updates.
